	...
	public class ExiledEventHandlers
	{
		...
		public void EtherealInterventionSpawn(SpawnedEventArgs ev)
		{
			if (PerkSystem.HasPerk(ev.Player, PerkType.EtherealIntervention) && ev.Reason == SpawnReason.None)
			{
				...
				IPerk blissfulUnawarenessPerk = PerkType.BlissfulUnawareness.ToPerk();
		
				if (PerkSystem.HasPerk(ev.Player, PerkType.BlissfulUnawareness)) PerkSystem.RemovePerk(ev.Player, blissfulUnawarenessPerk);
		...
		public void ActivatingPerk(TogglingNoClipEventArgs ev)
		{
			...
			{
				if (ev.Player.IsGodModeEnabled == false)
				{
					...
					if (!FoundationFortune.Instance.HintSystem.ConfirmActivatePerk.TryGetValue(ev.Player.UserId, out bool isConfirming) || !FoundationFortune.Instance.HintSystem.ActivatePerkTimestamp.TryGetValue(ev.Player.UserId, out float toggleTime)) continue;
					if (!isConfirming || !(Time.time - toggleTime <= PlayerDataRepository.GetActiveAbilityActivationTime(ev.Player.UserId))) continue;
