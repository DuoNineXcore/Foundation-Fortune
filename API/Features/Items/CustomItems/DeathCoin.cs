...
namespace FoundationFortune.API.Items.CustomItems
{
	...
	public class DeathCoin : CustomItem
	{
		...
		private void OnDeath(DyingEventArgs ev)
		{
			...
			FoundationFortune.Singleton.HintSystem.EnqueueHint(ev.Player, FoundationFortune.Singleton.Translation.Death.Replace("%moneyBeforeDeath%", moneyBeforeDeath.ToString()));
		...
		protected override void OnAcquired(Player player, Item item, bool displayMessage)
		{
			...
			if (droppedCoins.TryGetValue(item.Serial, out var coinData))
			{
				...
				FoundationFortune.Singleton.HintSystem.EnqueueHint(player, FoundationFortune.Singleton.Translation.DeathCoinPickup.Replace("%coinValue%", coinValue.ToString()));
