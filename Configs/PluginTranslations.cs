...
namespace FoundationFortune.Configs
{
    public class PluginTranslations : ITranslation
    {
        ...
        public string ItemConfirmation { get; set; } = "<b><size=24>This item is worth <color=green>${price}</color>, Confirm sale? ({GetConfirmationTimeLeft(ply)} seconds left)</size></b></align>";
        ...
        public string RevivalSuccess { get; set; } = "<b><size=24><color={reviver.Role.Color.ToHex()}>{reviver.Nickname}</color> Has Revived <color={reviver.Role.Color.ToHex()}>{targetToRevive.Nickname}</color></b></size>";
