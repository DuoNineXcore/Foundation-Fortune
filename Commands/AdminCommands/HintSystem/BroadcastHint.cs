...
namespace FoundationFortune.Commands.AdminCommands.HintSystem;

[CommandHandler(typeof(ClientCommandHandler))]
[CommandHandler(typeof(RemoteAdminCommandHandler))]
internal class BroadcastHint : ICommand,IUsageProvider
{
    public string Command { get; } = "ff_broadcasthint";
    ...
    public bool Execute(ArraySegment<string> args, ICommandSender sender, out string response)
    {
        ...
        switch (target)
        {
            case "self":
            {
                ...
                FoundationFortune.Instance.HintSystem.BroadcastHint(player, hint);
                response = "Hint broadcasted successfully.";
                break;
            }
            case "all":
            {
                foreach (Player ply in Player.List.Where(p => !p.IsNPC)) FoundationFortune.Instance.HintSystem.BroadcastHint(ply, hint);
                response = "Hint broadcasted successfully.";
                break;
            }
            default:
            {
                ...
                FoundationFortune.Instance.HintSystem.BroadcastHint(steamId, hint);
                response = "Hint broadcasted successfully.";
                break;
            }
        }
