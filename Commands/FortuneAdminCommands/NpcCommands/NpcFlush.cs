        ...
        public bool Execute(ArraySegment<string> args, ICommandSender sender, out string response)
        {
            ...
            if (args.Array != null)
            {
                ...
                if (Enum.TryParse(npcTypeString, true, out NpcType npcType))
                {
                    ...
                    {
                        case NpcType.Buying:
                            count = RemoveBots(FoundationFortune.Instance.BuyingBots, BuyingBot.RemoveBuyingBot);
                            response = $"Flushed {count} BuyingBots from the server.";
                            return true;
                        case NpcType.Selling:
                            count = RemoveBots(FoundationFortune.Instance.SellingBots, SellingBot.RemoveSellingBot);
