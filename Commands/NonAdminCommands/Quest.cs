...
public sealed class Quest : ICommand, IUsageProvider
{
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        {
            List<QuestType> randomQuests = QuestRotationRepository.GetShuffledQuestsForUser(player.UserId);
            ...
            string resetMessage = QuestRotationRepository.MaxRotations - QuestRotationRepository.RotationCounter == 1 ? "round" : "rounds";
            response = $"Choose a Quest by typing: {Command} 1-3\n{questList}\nQuests will reset in {QuestRotationRepository.MaxRotations - QuestRotationRepository.RotationCounter} {resetMessage}.";
            return true;
        }
        ...
        if (int.TryParse(arguments.At(0), out int selectedQuestNumber))
        {
            ...
            {
                QuestType selectedQuestType = QuestRotationRepository.GetShuffledQuestsForUser(player.UserId)[selectedQuestNumber - 1];
