	...
	public partial class ServerEvents
	{
		...
		private IEnumerator<float> ExtractionTimer(Player player, TimeSpan waitDuration)
		{
			...
			while (IsPlayerInExtractionRoom(player, activeExtractionRoom))
			{
				...
				{
					float transferAmount = (int)(totalMoneyOnHold * 0.1f) < 1 ? 1 : totalMoneyOnHold * 0.1f;

					Log.Debug(totalMoneyOnHold);
					Log.Debug(transferAmount);
		...
		private void StartExtractionTimer(Player player, TimeSpan duration)
		{
			...
			{
				CoroutineHandle timerHandle = Timing.RunCoroutine(ExtractionTimer(player, duration).CancelWith(player.GameObject));
